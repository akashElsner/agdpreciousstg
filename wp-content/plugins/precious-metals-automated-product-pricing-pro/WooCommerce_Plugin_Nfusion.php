...
define("NFS_CATALOG_PLUGIN_VERSION",   "3.0.3");
...
function is_not_fee_exempt($chosen_payment_method) {
	return !in_array($chosen_payment_method, array('cod', 'cheque', 'bacs'));
}
	...
	if(get_option('nfusion_show_credit_card_price') == 'yes') {
		...
		if (is_checkout() && !is_wc_endpoint_url()) {
			...
			if (is_not_fee_exempt($chosen_payment_method)) {
				// Get cc price percent from nFusion settings
				$order_total = $cart->subtotal_ex_tax;
				$percent_fee = $order_total * ($cc_percent / 100);

				// Add the percent fee to the cart
...
function nfusion_add_pricing_settings() {
	...
	add_settings_field(
		...
		'nfusion_render_field', 'nfusion_settings_page_pricing',
		'nfusion_pricing_settings_section', array('option_name' => 'nfusion_cc_price', 'type' => 'number', 'min' => '0')
	);
}
...
function nfusion_render_field($args) {
	...
	switch ($type) {
		...
		default:
			?>
            <input type="<?php echo $type; ?>" name="<?php echo $option_name; ?>" value="<?php echo esc_attr($value); ?>" <?php if (isset($args['min'])) echo 'min="' . esc_attr($args['min']) . '"'; ?> />
		<?php
	}
}
