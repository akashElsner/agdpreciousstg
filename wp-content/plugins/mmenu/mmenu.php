...
<pre>&lt;a id="my-button" href="#my-menu"&gt;open menu&lt;/a&gt;</pre>
	...
	*/
	...
	protected function saveFrontend( $fileAffix = '' )
	{
		...
		{
			$y = ',
				"(min-width: ' . $mm[ 'menu' ][ 'breakpoint' ] . ')": ["widescreen"]';
		}

		if ( count( $x ) > 0 || strlen( $y ) > 0 )
		{
			$o[] = '
			extensions: {
				"all": ["' . implode( '", "', $x ) . '"]' . $y . '
...
jQuery(document).ready(function($) {
	...
	$menu.add( $menu.find( "ul, li" ) ).removeAttr( "class" );

	$menu.filter( "[id]" )
		.add( $menu.find( "[id]" ) )
		.each(function() {
			$(this).attr( "id", "wpmm-" + $(this).attr( "id" ) );
		});
...
.wpmm-header-image
{
	...
	left: ' . $pos . ';
		...
		{
			$css .= '
@media (min-width:' . intval( $mm[ 'menu' ][ 'breakpoint' ], 10 ) . 'px) {
	.wpmm-button ,
	' . $mm[ 'setup' ][ 'menu' ] . '
	{
...
';
	...
	public function admin_css_js( $screen_id )
	{
		...
		if ( $screen_id == $this->screen_id )
		{
			...
			wp_register_script( 'mmenu', plugin_dir_url( __FILE__ ) . 'mmenu/jquery.mmenu.all.js' );
...
class MmenuFrontend {
	...
	public function enqueue_scripts()
	{
		...
		if ( current_user_can( 'manage_options' ) &&
			...
			{
				wp_enqueue_script( 	'mm-breakpoint', plugin_dir_url( __FILE__ ) . 'lib/breakpoint/admin-breakpoint.js', array( 'jquery' ) );
			}
